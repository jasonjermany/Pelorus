<template>
  <div class="min-h-screen bg-surface-50 font-sans">
    ...
    <main class="mx-auto max-w-5xl px-8 py-8 flex flex-col gap-4">
      <!-- Upload Card -->
      <div
        class="bg-white rounded-2xl border border-black/[0.07] shadow-card p-7"
      >
        <div class="flex items-start justify-between gap-6 mb-5">
          <div>
            <h2
              class="font-sans text-[21px] text-primary-800 tracking-[-0.5px] mb-1.5"
            >
              Upload Guidelines
            </h2>
            <p
              class="text-[13px] text-black/45 leading-relaxed font-light max-w-[480px]"
            >
          ...
          <div v-if="chunks.length" class="flex gap-6 flex-shrink-0">
            <div class="text-right">
              <span
                class="block font-sans text-[26px] text-primary-800 tracking-[-1px] leading-none"
                >{{ standardCount }}</span
              >
              <span class="block text-[11px] text-black/30 mt-1"
                >Guideline sections</span
              >
            </div>
            <div class="text-right">
              <span
                class="block font-sans text-[26px] text-danger-700 tracking-[-1px] leading-none"
                >{{ pinnedCount }}</span
              >
              <span class="block text-[11px] text-black/30 mt-1"
                >Hard stops</span
        ...
        >
          ⚠ Uploading a new PDF will replace all existing guidelines for this
          organization.
        </div>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            ...
            />
            <div
              class="flex items-center gap-2.5 px-4 py-3 pointer-events-none"
            >
              <svg
                width="18"
                height="18"
                viewBox="0 0 24 24"
                fill="none"
                :class="selectedFile ? 'text-primary-800' : 'text-black/25'"
              >
                <path
        ...
        <div
          v-if="isUploading"
          class="flex items-center gap-2.5 mt-4 text-[12px] text-black/40"
        >
          <span
            class="inline-block w-3.5 h-3.5 border-2 border-accent-500/30 border-t-accent-500 rounded-full animate-spin flex-shrink-0"
          />
          Parsing document, extracting hard stops, generating embeddings — this
          may take 2–3 minutes.
        </div>

        <p v-if="uploadError" class="mt-3 text-[13px] text-danger-700">
          {{ uploadError }}
        </p>
        <p v-if="uploadSuccess" class="mt-3 text-[13px] text-success-700">
          {{ uploadSuccess }}
        </p>
      </div>
      ...
      <div
        v-if="chunks.length"
        class="bg-white rounded-2xl border border-black/[0.07] shadow-card overflow-hidden"
      >
        <div
          class="flex items-center justify-between px-6 py-4 border-b border-black/[0.05]"
        >
          <div>
            <h2 class="text-[14px] font-semibold text-primary-800">
              Guideline Library
            </h2>
            <p class="text-[12px] text-black/30 mt-0.5">
              {{ chunks.length }} total · {{ standardCount }} sections ·
              {{ pinnedCount }} hard stops
            </p>
          </div>
          <button
        ...
        <div v-if="loadError" class="px-6 py-4 text-[13px] text-danger-700">
          {{ loadError }}
        </div>
        ...
        <table class="w-full text-left text-[13px]">
          <thead>
            <tr class="border-b border-black/[0.05]">
              <th
                class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.1em] text-black/25 w-16"
              >
                Page
              </th>
              <th
                class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.1em] text-black/25"
              >
                Preview
              </th>
              <th
                class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.1em] text-black/25 w-28"
              >
                Type
              </th>
              <th
                class="px-5 py-3 text-[10px] font-bold uppercase tracking-[0.1em] text-black/25 w-24"
              >
                Added
              </th>
            </tr>
          ...
          <tbody class="divide-y divide-black/[0.04]">
            ...
            >
              <td class="px-5 py-3 text-[12px] text-black/25 align-top">
                {{ chunk.page ?? "—" }}
              </td>
              <td
                class="px-5 py-3 align-top cursor-pointer select-none"
                @click="expandedId = expandedId === chunk.id ? null : chunk.id"
              >
                <span class="flex items-start gap-1.5">
                  <svg
                    class="mt-0.5 flex-shrink-0 text-black/20 transition-transform"
                  ...
                  <span
                    v-if="expandedId === chunk.id"
                    class="text-[12px] text-black/60 whitespace-pre-wrap leading-relaxed"
                    >{{ getCleanText(chunk.embed_text) }}</span
                  >
                  <span v-else class="text-[12px] text-black/50">{{
                    getChunkPreview(chunk.embed_text)
                  }}</span>
                </span>
              </td>
              ...
              <td class="px-5 py-3 text-[11px] text-black/25 align-top">
      ...
      <div
        v-else-if="!loadError"
        class="bg-white rounded-2xl border border-black/[0.07] shadow-card py-20 flex flex-col items-center gap-3"
      >
        <svg
          width="40"
          height="40"
        ...
        <p class="text-[14px] font-semibold text-black/40">
          No guidelines uploaded yet
        </p>
        <p class="text-[13px] text-black/25 font-light">
          Upload your carrier's underwriting guidelines PDF to get started.
        </p>
      </div>
    </main>
  </div>
...
const pinnedCount = computed(
  () => chunks.value.filter((c) => c.is_pinned).length,
);
...
async function loadChunks() {
  ...
  try {
    const { chunks: data } = await $fetch<{ chunks: GuidelineChunk[] }>(
      "/api/guidelines",
    );
    chunks.value = data ?? [];
  } catch (e: any) {
...
async function uploadGuidelines() {
  ...
  try {
    ...
    const result = await $fetch<{
      ...
      hard_stops_stored: number;
    }>("/api/guidelines/upload", { method: "POST", body: fd });
